<template>
  <div>
    <el-button type="primary" @click="openCreate">Create Category</el-button>
    ...
    <el-dialog
      ...
      @close="resetForm"
    >
      <category-form ref="childRef" :model="editing" />
      <template #footer>
...
const load = async () => {
  try {
    ...
    ElMessage.error('Load categories failed')
  }
}
...
const submitForm = async () => {
  ...
  try {
    ...
    if (payload.id) {
      await axios.put(`/categories/${payload.id}`, { name: payload.name })
    } else {
      await axios.post('/categories', { name: payload.name })
    }
...
const confirmDelete = (row: Meta) => {
  ElMessageBox.confirm(`Delete category "${row.name}"?`, 'Confirm', { type: 'warning' })
    .then(async () => {
      try {
